...
def write_images(images, labels, Dataset_dir, t):
    img_save_dir = Dataset_dir + t + '/'
    if not os.path.isdir(img_save_dir):
        os.mkdir(img_save_dir)

    labels_file = img_save_dir + t + "-labels.csv"
    ...
    with open(labels_file, 'wb') as csvFile:
        ...
        for i in range(len(images)):
            img = images[i]
            plt.imsave(arr=img[0], fname=img_save_dir + str(i) + ".png", cmap='gray')
            writer.writerow([str(i) + ".jpg", labels[i][0]])
...
def main():
    Dataset_dir = '/home/mikep/DataSets/EMNIST/'
    path = Dataset_dir + 'matlab/emnist-bymerge.mat'
    ...
    if not os.path.isdir(img_dir):
        os.mkdir(img_dir)
    write_images(x_train, y_train, img_dir, "train")
    write_images(x_test, y_test, img_dir, "test")
    ...
    train_labels_file = train_dir + "train-labels.csv"
    test_labels_file = test_dir + "test-labels.csv"
    ...
    _process_dataset("train", train_dir, train_shards, train_labels_file, num_threads, tfrecord_dir)
    _process_dataset("test", test_dir, train_shards, test_labels_file, num_threads, tfrecord_dir)
